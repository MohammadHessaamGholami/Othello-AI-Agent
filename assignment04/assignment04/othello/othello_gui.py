...
from ai import select_move_alphabeta
...
class OthelloGui(object):
    ...
    def __init__(self, game_manager, player1, player2):
        ...
        self.canvas = Canvas(root, height=self.cell_size * self.height +
                             self.offset, width=self.cell_size * self.width + self.offset)
    ...
    def ai_move(self):
        ...
        #print(self.game.board[1][1])
        player_obj = self.players[self.game.current_player]
        try:
            #print("////////////")
            #print(self.game.board)
            #print(get_possible_moves(self.game.board,1))
            #movei, movej = select_move_alphabeta(self.game.board, 1)
            #print(movei)
            #print(movej)
            #print("**")
            #print(self.game.board)
            #print(get_possible_moves(self.game.board,1))
            #new_board=play_move(self.game.board,1,1,0)
            #print(get_score(self.game.board))
            #print(new_board)
            ...
            self.shutdown(
                "Game Over, {} lost (timeout)".format(player_obj.name))
        ...
        for i in range(self.height):
            for j in range(self.width):
                self.canvas.create_rectangle(i*self.cell_size + self.offset, j*self.cell_size + self.offset,
                                             (i+1)*self.cell_size + self.offset, (j+1)*self.cell_size + self.offset, fill="dark green")
    ...
    def draw_disk(self, i, j, color):
        ...
        self.canvas.create_oval(
            x+padding, y+padding, x+self.cell_size-padding, y+self.cell_size-padding, fill=color)
